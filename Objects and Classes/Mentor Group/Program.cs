...
namespace Mentor_Group
{
    class Program
    {
        ...
        {
            Dictionary<string, StudentData> students = new Dictionary<string, StudentData>();

            string currentDate = Console.ReadLine();

            while (currentDate != "end of dates")
            {
                string[] arguments = currentDate.Split(' ', ',').ToArray();

                string name = arguments[0];

                string[] datesArray = arguments.Skip(1).ToArray();
            ...
            {
                string[] arguments = currentComment.Split('-').ToArray();
            ...
            var studentsFormatted = students.OrderBy(n => n.Key).ToList();
